...
def makeDictFromCsv(f):
    """
    Reads .csv file, temporarily encodes the input unicode in UTF-8, and creates a unicode dictionary.
    ...
    with open(f, 'rb') as csvfile:
        ...
        with open('cleanDict.txt', 'wb') as dictfile:
...
def filterMetadataUsingFiles(fileList):
    ...
    """
    # Replace sys.argv[1] with 'all-metadata_REUPS.csv' when running in pyCharm
    outputcsvfile = open('filtered_' + 'all-metadata_REUPS_unicode.csv', 'wb')
